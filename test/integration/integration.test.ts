...
describe("mcp schema + parity integration", () => {
  ...
  it("validates tools/list JSON schema structure", async () => {
    const toolsListRaw = await dispatchMcpRequest(
      JSON.stringify({
        jsonrpc: "2.0",
        id: 11,
        method: "tools/list",
  ...
  it("keeps CLI and MCP execution parity for compute_flip_distance", async () => {
    ...
    const expected = executeZeoliteOperation("compute_flip_distance", {
      contextId,
    });
    ...
    const resultRaw = await dispatchMcpRequest(
      JSON.stringify({
        jsonrpc: "2.0",
        id: 2,
        method: "tools/call",
...
describe("transcript CLI and MCP parity", () => {
  it("verifies and replays exported transcript", async () => {
    await withTempCwd(async (cwd) => {
      ...
      const contextId = String(loaded.contextId);
      const exported = executeZeoliteOperation("export_transcript", {
        contextId,
