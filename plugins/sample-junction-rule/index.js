      ...
      decisionTypes: [
        {
          ...
          createJunction(context) {
            const { service, current_version, target_version, risk_tolerance, traffic_pattern } = context || {};
          ...
          evaluate(junction, policies) {
            ...
            const validOptions = junction.options.filter(opt => 
              opt.confidence >= (junction.selection_criteria?.minimum_confidence || 0.6)
            );
            ...
            if (policies) {
              policies.forEach(policy => {
                if (policy.id === "require-rollback-fast") {
                  const slowRollback = validOptions.filter(o => 
                    o.constraints.rollback_time === "slow"
                  );
