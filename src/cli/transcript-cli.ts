...
async function loadTranscriptSecurity(): Promise<TranscriptSecurityModule> {
  const fallback = new URL(
    "../../../packages/core/src/transcript-security.js",
    import.meta.url,
  ).href;
  try {
...
export async function runTranscriptCommand(argv: string[]): Promise<number> {
  ...
  try {
    ...
    if (entity === "keygen") {
      const keyPath =
        value(argv, "--out") ??
        join(process.cwd(), ".zeo", "keys", "id_ed25519.pem");
    ...
    if (entity === "transcript" && action === "countersign") {
      ...
      const statement =
        value(argv, "--statement") ?? "I disagree unless condition holds.";
      if (!input || !signer || !condition)
        throw new Error(
          "Usage: zeo transcript countersign <envelope.json> --signer <id> --if <condition>",
        );
      const envelope = loadEnvelopeFromFile(input) as unknown as Record<
        string,
        unknown
      >;
      const metadata =
      ...
      process.stdout.write(
        `${JSON.stringify({ out, objections: objections.length }, null, 2)}\n`,
      );
      return 0;
    }
    ...
    if (entity === "transcript" && action === "disagreements") {
      const input = argv[2];
      if (!input)
        throw new Error("Usage: zeo transcript disagreements <envelope.json>");
      const envelope = loadEnvelopeFromFile(input) as unknown as Record<
        string,
        unknown
      >;
    ...
    if (entity === "transcript" && action === "consensus") {
      const input = argv[2];
      if (!input)
        throw new Error("Usage: zeo transcript consensus <envelope.json>");
      const envelope = loadEnvelopeFromFile(input) as unknown as Record<
        string,
        unknown
      >;
      ...
      const summary = {
        ...
        signatures: Array.isArray(envelope.signatures)
          ? envelope.signatures.length
          : 0,
        disagreements: disagreements.length,
        structural_consensus:
          disagreements.length === 0 ? "full" : "conditional",
      };
    ...
    if (entity === "transcript" && action === "verify") {
      ...
      const keyring =
        value(argv, "--keyring") ?? join(process.cwd(), ".zeo", "keyring");
    ...
    if (entity === "transcript" && action === "inspect") {
      const input = argv[2];
      if (!input)
        throw new Error("Usage: zeo transcript inspect <envelope.json>");
      process.stdout.write(
    ...
    if (entity === "transcript" && action === "chain" && argv[2] === "verify") {
      ...
      const envelopes = envelopeFilesInDir(resolve(dir)).map(
        loadEnvelopeFromFile,
      );
    ...
    if (entity === "transcript" && action === "qr") {
      ...
      process.stdout.write(
        `${JSON.stringify({ payload, qr_ascii: qrAscii(payload) }, null, 2)}\n`,
      );
      return 0;
    }
    ...
    if (entity === "transcript" && action === "qr-verify") {
      ...
      if (!payloadFile)
        throw new Error("Usage: zeo transcript qr-verify <payload.json>");
      const payload = JSON.parse(
        readFileSync(resolve(payloadFile), "utf8"),
      ) as Record<string, unknown>;
    ...
    if (entity === "keys" && action === "list") {
      const keyringDir =
        value(argv, "--keyring") ?? join(process.cwd(), ".zeo", "keyring");
      process.stdout.write(
        `${JSON.stringify(listKeyringEntries(resolve(keyringDir)), null, 2)}\n`,
      );
      return 0;
    }
    ...
    if (entity === "trust" && action === "record") {
      ...
      if (!envPath)
        throw new Error("Usage: zeo trust record --from <envelope.json>");
      const root = process.cwd();
      const keyringDir =
        value(argv, "--keyring") ?? join(root, ".zeo", "keyring");
      const envelope = loadEnvelopeFromFile(envPath);
      const verify = verifyEnvelope(
        envelope,
        keyringResolver(resolve(keyringDir)),
      );
    ...
    if (entity === "trust" && action === "show") {
      ...
      if (!subject)
        throw new Error("Usage: zeo trust show <subject_type:subject_id>");
      const profiles = compactTrustProfiles(process.cwd());
      const match = profiles.find(
        (p) => `${p.subject_type}:${p.subject_id}` === subject,
      );
    ...
    if (entity === "trust" && action === "compact") {
      process.stdout.write(
        `${JSON.stringify(compactTrustProfiles(process.cwd()), null, 2)}\n`,
      );
      return 0;
    }
