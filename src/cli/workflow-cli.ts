      ...
      provenance: [
        {
          ...
          capturedAt: e.assertedAt || ws.createdAt || nowIso(),
          checksum: e.provenance.hash,
        },
      ],
      tags: [] as string[],
    })),
...
function requiredEvidenceCountForType(type: DecisionType): number {
  const map: Record<DecisionType, number> = {
    ENG: 2,
    OPS: 2,
    SEC: 3,
    PROD: 2,
...
function computeDecisionHealth(ws: DecisionWorkspace, replayStable: boolean): DecisionHealth {
  ...
  const expired = ws.evidence.filter(
  ...
  const riskScore = clampScore(
    Math.round(
      (100 - evidenceScore) * 0.35 +
        (100 - policyScore) * 0.35 +
        (100 - replayStabilityScore) * 0.2 +
        volatility * 0.1,
    ),
  );
...
function healthLine(health: DecisionHealth): string {
  const composite = clampScore(
    Math.round(
      (health.evidenceCompletenessScore +
        health.policyComplianceScore +
        health.replayStabilityScore +
        (100 - health.assumptionVolatilityIndex)) /
        4,
    ),
  );
...
function nextSteps(
  context: "analyze-pr" | "decision-finalize" | "verify-mismatch",
...
function verificationStatusFromHealth(
  health: DecisionHealth,
  manifestHash: string,
  treeHash: string,
  signed: boolean,
...
function parseNoteToEvidence(
  ...
  return {
    ...
    cost: {
      timeMinutes: Math.max(5, Math.ceil(cleaned.length / 80) * 5),
      risk: cleaned.includes("urgent") ? "high" : "low",
    },
  };
}
...
function formatResultCard(result: RunResult): string {
  return [
    ...
    "Top evidence:",
    ...result.topEvidence.map(
      (e, idx) =>
        `  ${idx + 1}. ${e.summary} (time=${e.cost.timeMinutes}m risk=${e.cost.risk} decay=${e.decay})`,
    ),
    "Regret-bounded plan:",
...
async function runDecisionInWorkspace(
  ...
  return {
    ...
    signatureStatus:
      envelopePathOut || (envelopePath && existsSync(resolve(envelopePath)))
        ? "signed"
        : "unsigned",
...
function createTasksFromEvidence(evidence: EvidenceItem): TaskItem[] {
  return [
    {
      id: `task_${evidence.id}`,
      label: `Validate: ${evidence.summary}`,
...
function collectGraph(_asOfDate: string): {
  ...
  for (const ws of collectWorkspaces()) {
    for (const run of ws.runs) {
      nodes.set(run.transcriptHash, {
        transcriptHash: run.transcriptHash,
        decisionId: ws.decisionId,
        flipDistance: run.flipDistance,
      });
      for (const parent of run.dependsOn ?? [])
        edges.push({
          from: parent,
          to: run.transcriptHash,
          type: "depends_on",
        });
      for (const child of run.informs ?? [])
        edges.push({ from: run.transcriptHash, to: child, type: "informs" });
    }
  }
...
function buildTypeSummary(
  ...
  const text =
    rows
      .map(
        (row) =>
          `${row.decisionId} type=${row.type} mode=${row.mode} state=${row.state} reviewAfter=${row.reviewAfter} evidence=${row.evidenceCount} audience=${audience}`,
      )
...
export async function runWorkflowCommand(args: WorkflowArgs): Promise<number> {
  ...
  if (args.command === "decision" && args.subcommand === "create") {
    ...
    const assumptions = template.requiredAssumptions
      ...
      .map((proposal, index) => ({
        ...proposal,
        id: `ev_${hash({ decisionId, index, summary: proposal.summary, kind: "assumption" }).slice(0, 10)}`,
      }));
    ...
    writeJsonOrText(
      args,
      { decisionId, template, reviewAt },
      `Started decision '${title}' (${decisionId}) from template ${template.id}. reviewAt=${reviewAt}`,
    );
    return 0;
  }
  ...
  if (args.command === "roi-report") {
    ...
    const payload = {
      ...
      decisionsCount,
      avgTimeToDecision,
      percentWithFullEvidence,
      securityFlagsPreMerge,
      claimsBackedByEvidence,
    };
    writeJsonOrText(
      args,
      payload,
      `window=${window} decisions=${decisionsCount} avg_time_min=${avgTimeToDecision} full_evidence=${percentWithFullEvidence}% security_flags=${securityFlagsPreMerge} claims_backed=${claimsBackedByEvidence}%`,
    );
    return 0;
  }
  ...
  if (args.command === "start") {
    ...
    const ws: DecisionWorkspace = {
      decisionId,
      title,
      decisionType: args.type ?? "ENG",
      workspaceMode: args.mode ?? "customer",
      state: "proposed",
      createdAt,
      evidence: [],
      tasks: [],
  ...
  if (args.command === "evidence" && args.subcommand === "expired") {
    ...
    const expired = collectWorkspaces().flatMap((ws) =>
      ws.evidence
        .filter((e) => classifyDecay(e, today) === "expired")
        .map((e) => ({
          decisionId: ws.decisionId,
          evidenceId: e.id,
  ...
  if (args.command === "graph") {
    ...
    if (args.subcommand === "impact") {
      ...
      writeJsonOrText(
        args,
        { transcript: transcriptHash, impact },
    ...
    if (args.subcommand === "fragility") {
      const ranked = graph.nodes
        .map((node) => ({
          ...
          fragilityScore:
            downstreamImpact(graph, node.transcriptHash).length *
            Math.max(1, 10 - node.flipDistance),
        }))
        .sort(
          (a, b) =>
  ...
  if (args.command === "evidence" && args.subcommand === "set-expiry") {
    ...
    writeJsonOrText(
      args,
      { decisionId, evidenceId: evidence.id, expiresAt },
      `Set expiry for ${evidence.id} to ${expiresAt}`,
    );
    return 0;
  }
  ...
  if (args.command === "add-note") {
    ...
    const proposal = parseNoteToEvidence(
      text,
      isoDate(args.assertedAt) ?? undefined,
      isoDate(args.expiresAt) ?? undefined,
    );
    const evidence: EvidenceItem = {
      ...proposal,
      id: `ev_${proposal.provenance.hash.slice(0, 10)}`,
    };
    ...
    writeJsonOrText(
      args,
      { proposal, accepted: true, evidenceId: evidence.id },
      `Added evidence ${evidence.id}: ${evidence.summary}`,
    );
    return 0;
  }
  ...
  if (args.command === "run") {
    ...
    const result = await runDecisionInWorkspace(
      ws,
      args.envelope,
      asOf,
      args.dependsOn,
    ...
    const replayStable =
      ws.runs.length === 0
        ? true
        : ws.runs[ws.runs.length - 1].transcriptHash === result.transcriptHash;
  ...
  if (args.command === "share" || args.command === "copy") {
    const latest =
      ws.runs[ws.runs.length - 1] ??
      (await runDecisionInWorkspace(
        ws,
        args.envelope,
        isoDate(args.asOf) ?? DEFAULT_AS_OF_DATE,
        [],
        [],
      ));
  ...
  if (args.command === "export") {
    ...
    if (args.subcommand === "decision") {
      ...
      const replay = {
        transcriptHash: latest.transcriptHash,
        expectedHash: latest.transcriptHash,
      };
      const policy = {
      ...
      const status = verificationStatusFromHealth(
        health,
      ...
      const manifestPayload = {
        schemaVersion: "1.0.0",
        files: buildBundleManifest(folder, bundleFiles).files,
      ...
      const proofLine = status.verified
    ...
    if (args.subcommand === "bundle") {
      ...
      if (args.envelope && existsSync(resolve(args.envelope)))
        cpSync(resolve(args.envelope), join(folder, "envelope.json"));
      writeFileSync(
        join(folder, "summary.md"),
        `# ${ws.title}\n\n${formatResultCard(latest)}\n`,
        "utf8",
      );
      writeFileSync(
        join(folder, "trust-snapshot.json"),
        `${JSON.stringify({ decisionId: ws.decisionId, runCount: ws.runs.length }, null, 2)}\n`,
        "utf8",
      );
  ...
  if (args.command === "explain") {
    ...
    if (!target)
      throw new Error(
        "Usage: zeo explain --decision <id> [--audience legal|exec|sales|engineer|auditor]",
      );
