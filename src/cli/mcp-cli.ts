...
const TOOL_DEFS: McpToolDef[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      required: ["contextId", "transcriptId"],
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      required: ["contextId", "sourceId", "claim"],
    },
  },
  {
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { contextId: { type: "string" }, agentClaim: {} },
      required: ["contextId"],
    },
  },
];
...
export function validateMcpToolDefinitions(
  ...
  for (const def of definitions) {
    ...
    if (!Array.isArray(def.inputSchema.required))
      issues.push(`tool '${def.name}' inputSchema.required must be an array`);
  }
...
async function runServeCommand(args: McpCliArgs): Promise<number> {
  ...
  rl.on("line", (line) => {
    ...
    if (!trimmed) return;
    void dispatchMcpRequest(trimmed).then((out) => {
