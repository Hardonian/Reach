    ...
    const recommendations = dirs
      .map((dir) => {
        ...
        return {
          agent: manifest.id,
          capabilityMatches,
          acceptanceRatio: Number(acceptanceRatio.toFixed(4)),
          score,
        };
      })
  ...
  if (args.command === "add") {
    ...
    if (!args.accept) {
      ...
      if (args.json) process.stdout.write(`${JSON.stringify(msg, null, 2)}\n`);
        ...
        console.log(
          `Agent ${manifest.id} requests permissions fs=${permissions.fs} network=${permissions.network}. Re-run with --accept.`,
        );
      return 1;
    }
    if (manifest.permissions.fs || manifest.permissions.network) {
      throw new Error(
        "Agent permissions exceed default sandbox. Set fs/network to false and use explicit runtime escalation.",
      );
    }
