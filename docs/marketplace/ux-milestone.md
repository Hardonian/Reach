# Reach Marketplace UX Milestone

## API endpoints

### `GET /v1/marketplace/catalog`
Supports `q`, `kind`, `risk`, `tier`, `capability`, `tag`, `publisher`, `sort`, `page`, `page_size`, `verified`.
...
### `GET /v1/marketplace/items/{kind}/{id}`
Returns full item detail including trust metadata and capability disclosure.

### `POST /v1/marketplace/install-intent`
Request:
...
## Mobile navigation map (Android and iOS parity)
...
- Home: search, category chips, verified toggle, risk/tier filters.
...
## VS Code command list and interaction flow

1. `Reach: Marketplace Search` (`reach.marketplaceSearch`) opens input, loads catalog into QuickPick, then consent-confirmed install.
...
## Catalog caching strategy and limits

- Server-side catalog cache with ETag/If-None-Match and Last-Modified/If-Modified-Since when remote index supports validators.
...
- No auto-install: install endpoint requires explicit consent payload.
