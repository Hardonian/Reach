# Reach Quick Start Guide (Non-Technical)
...
```bash
# Using Docker
docker run -d -p 8787:8787 -v reach-data:/data --name reach reach/reach:latest
...
### Step 3: Run a Demo Workflow

```bash
# Create a new run
reach run --pack arcadeSafe.demo
```
...
A "time capsule" is a snapshot of a run that you can save for later verification:

```bash
# Create a capsule from your run
reach capsule create <run-id>

# Verify the capsule later
reach capsule verify <path-to-capsule>
```

## Replaying a Run

You can replay any run to verify it produces the same results:
...
By default, Reach stores everything in:
...
```bash
# Docker
docker run -p 8787:8787 -v /my/custom/path:/data reach/reach:latest

# CLI
reach serve --data /my/custom/path
```
...
Your user doesn't have permission to write to the data directory:
...
- **Browse available packs**: `reach packs search`
...
Reach is designed with safety in mind:
