...
Use `reach release-check` before cutting any release.
...
## Security guardrails in release-check path
- Signature and SHA verification remain mandatory in non-dev mode.
...
## CI
`ci.yml` runs `./reach release-check` in `hardening-gates` on pull requests.
