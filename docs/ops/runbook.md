# Orchestration Runbook ## Diagnose stuck queues
...
## Dead-letter handling 1. Review `jobs.status='dead_letter'` and `last_error`.
2. Patch policy/capability/tier misconfiguration.
3. Re-enqueue with a new idempotency key after corrective action.

## Node offline behavior 1. Heartbeat endpoint updates `last_heartbeat_at` and `status`.
...
## Tier errors Restricted orchestration actions return:
- `error = tier_required`
