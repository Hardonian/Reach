# Reach Mobile Operator Guide

One-tap execution for Android/Termux. Run → Verify → Share in 3 steps.
...
# Or with wget
wget -qO- https://get.reach.dev/termux | bash
```
...
| Command | Purpose | Mobile-Optimized |
|---------|---------|------------------|
...
Mobile mode automatically enables:
...
- **Clear language**: No technical jargon in wizard
...
```bash
# Reduce memory limit
export REACH_MAX_MEMORY_MB=128
reach wizard
```
...
- **Offline-first**: No network required for core operations
...
# Support bot
reach support ask "how do I share a run?"
