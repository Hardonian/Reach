...
Reach uses a single internal event stream as the source of truth for replay, proofs, graph exports, explain output, arena scoring, operator telemetry, and gamification hooks.

## Canonical stream contract
- Events are JSON payloads validated by event type before storage.
...
- Event logs are persisted and consumed as ordered arrays.
...
- Support and explain surfaces are constrained to safe/templated responses.
...
- Payload normalizer preserves compatibility by defaulting missing `schemaVersion` to `1.0.0`.
- Explicit mismatched versions are rejected instead of silently coercing, preventing replay drift.

## Event hooks used by product features
The following hooks are consumed by the gamification store and scoreboards:
- `onRunCompleted`
- `onRunFailed`
- `onPolicyDenied`
- `onReplayVerified`
