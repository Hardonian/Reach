...
Reach mesh nodes identify with:
- `nodeId` (stable UUID)
- `orgId`
...
## Protocol
1. Acceptor emits challenge `{nonce, policyVersion, registrySnapshotHash, issuedAt}`.
...
## Security
- Replay prevention is enforced by per-node signature memoization for the handshake TTL.
