...
This document defines the normative execution contract used by runner orchestration, replay, policy admission, audit, and federation delegation.

## 2. Normative contracts

### 2.1 Execution graph rules
...
### 2.2 Pack signing contract

1. A pack MUST include a signature hash over canonical JSON of all pack fields excluding `signature_hash`.
...
### 2.3 Policy gate contract

1. Policy evaluation MUST deny undeclared tools.
...
### 2.4 Deterministic replay rules

1. Replay execution MUST validate `context.pack_hash` against loaded pack hash when provided.
...
### 2.5 Audit emission guarantees

1. Pack admission and denial decisions MUST emit deterministic audit events.
...
### 2.6 Federation delegation contract

1. Delegation MUST reject recursion to origin node.
...
### 2.7 Error state guarantees

1. Contract violations MUST return stable error codes.
...
- The canonical runtime execution spec version is `1.0.0`.
