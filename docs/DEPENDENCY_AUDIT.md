# Dependency Audit (Security, License, Supply Chain)
...
- **Rust**: workspace crates with `Cargo.lock` at repository root.
...
## Vulnerable package scan findings
...
- `go mod verify` in `services/runner`: **all modules verified**.
...
- `cargo audit` command is **not installed** in the environment (`error: no such command: audit`), so Rust advisory DB scan did not run here.

## License conflict check

- Top-level repository license: Apache-2.0 (`LICENSE`).
...
1. **Polyglot surface area** increases patch and advisory response burden.
...
1. Add CI-enforced per-ecosystem scans:
...
- **Security risk**: **PARTIAL** (root Node clean, but Rust/Go full advisory visibility incomplete in this runtime).
