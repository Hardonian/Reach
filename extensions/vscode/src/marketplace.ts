...
export async function runMarketplaceSearch(client: MarketplaceClient): Promise<void> {
  const query = await vscode.window.showInputBox({ prompt: 'Search Reach Marketplace' });
  if (!query) {
    return;
  }
  const results = await client.search(query);
  const selected = await vscode.window.showQuickPick(results.map((item) => ({
...
export async function runMarketplaceInstall(client: MarketplaceClient): Promise<void> {
  const kind = await vscode.window.showQuickPick(['connector', 'template', 'policy'], { title: 'Install kind' });
  if (!kind) {
    return;
  }
...
export async function runMarketplaceInstalled(client: MarketplaceClient): Promise<void> {
  ...
  await vscode.window.showQuickPick(items.map((item) => ({
    label: item.id,
...
export async function runMarketplaceUpdate(client: MarketplaceClient): Promise<void> {
  ...
  const target = await vscode.window.showQuickPick(items.map((item) => ({
    label: item.id,
