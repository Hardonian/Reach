...
async function loadBrowserWasm(): Promise<WasmModule> {
  ...
  const wasmUrl =
    (globalThis as unknown as { WASM_URL?: string }).WASM_URL || "/pkg/decision_engine.js";
...
function extractWasmExports(wasm: Record<string, unknown>): WasmModule {
  ...
  const exports = (wasm.default || wasm) as Record<string, unknown>;

  // Extract the functions we need
  const compute_fingerprint_json = (exports.compute_fingerprint_json ||
    wasm.compute_fingerprint_json) as ((input: string) => string) | undefined;
  ...
  return {
    compute_fingerprint_json: (input: string): string => {
      try {
        ...
        throw new Error(
          `WASM compute_fingerprint_json failed: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    },
    evaluate_decision_json: (input: string): string => {
      ...
      try {
        ...
        throw new Error(
          `WASM evaluate_decision_json failed: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    },
    get_engine_version: (): string => {
...
export async function computeFingerprintViaRust(preimage: unknown): Promise<string> {
  ...
  if (isWasmSuccess(result)) {
    ...
    throw new Error(
      `Rust fingerprint computation failed: ${result.error.code} - ${result.error.message}`,
    );
  }
}
