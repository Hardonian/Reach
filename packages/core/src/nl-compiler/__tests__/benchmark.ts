...
import { computeFingerprintSync, computeFingerprint, isWasmAvailable, resetWasmModule } from '../determinism-bridge.js';
...
async function runBenchmarks() {
  ...
  results.push(await benchmark('stableStringify (small)', () => stableStringify(smallData)));
  results.push(await benchmark('computeFingerprintSync (small)', () => computeFingerprintSync(smallData)));

  // Medium object benchmarks
  const mediumData = generateTestData('medium');
  results.push(await benchmark('stableStringify (medium)', () => stableStringify(mediumData)));
  results.push(await benchmark('computeFingerprintSync (medium)', () => computeFingerprintSync(mediumData)));

  // Large object benchmarks
  const largeData = generateTestData('large');
  results.push(await benchmark('stableStringify (large)', () => stableStringify(largeData), { iterations: 100 }));
  results.push(await benchmark('computeFingerprintSync (large)', () => computeFingerprintSync(largeData), { iterations: 100 }));

  // Key ordering benchmark
  const unorderedData = { z: 1, y: 2, x: 3, w: 4, v: 5, u: 6, t: 7, s: 8, r: 9, q: 10 };
  results.push(await benchmark('stableStringify (unordered keys)', () => stableStringify(unorderedData)));
  ...
  results.push(await benchmark('stableStringify (deeply nested)', () => stableStringify(nestedData)));

  // Array benchmark
  const arrayData = Array.from({ length: 100 }, (_, i) => ({ id: i, value: `item-${i}` }));
  results.push(await benchmark('stableStringify (array of objects)', () => stableStringify(arrayData)));
  ...
  if (wasmAvailable) {
    ...
    results.push(await benchmark('computeFingerprint (WASM, small)', () => computeFingerprint(smallData)));
    results.push(await benchmark('computeFingerprint (WASM, medium)', () => computeFingerprint(mediumData)));
    results.push(await benchmark('computeFingerprint (WASM, large)', () => computeFingerprint(largeData), { iterations: 100 }));
  } else {
