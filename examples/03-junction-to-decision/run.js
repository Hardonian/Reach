...
function main() {
  ...
  log(
    `Context: ${junction.context.service} v${junction.context.current_version} → v${junction.context.target_version}`,
  );
  log();
  ...
  junction.options.forEach((opt, i) => {
    ...
    log(
      `   Cost: ${opt.constraints.infrastructure_cost}, Rollback: ${opt.constraints.rollback_time}`,
    );
    log();
  });
