...
export function seed(): { success: boolean; message: string } {
  ...
  for (const [filename, scenario] of Object.entries(scenarios)) {
    writeFileSync(resolve(dataDir, filename), JSON.stringify(scenario, null, 2));
  ...
  for (const [name, scenario] of Object.entries(scenarios)) {
    ...
    console.log(`     Evidence: ${scenario.nodes.filter((n) => n.type === "evidence").length}`);
    console.log(`     Hypotheses: ${scenario.nodes.filter((n) => n.type === "hypothesis").length}`);
