...
function hashEvents(events) {
  ...
  return 'sha256:' + events.map(e => e.sequence + e.type).join('|').slice(0, 32);
}
...
function main() {
  ...
  const expectedParity = JSON.parse(readFileSync(resolve(EXAMPLE_DIR, 'expected-parity.json'), 'utf8'));
  ...
  log(`Integrity: manifest=${certificate.integrity.manifest}, events=${certificate.integrity.events}`);
  log();
