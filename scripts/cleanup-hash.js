...
for (let file of files) {
  ...
  content = content.replace(/import\s+\{\s*createHash,\s*(generateKeyPairSync|randomUUID)\s*\}\s*from\s+[\"']node:crypto[\"'];\r?\n/, 'import { $1 } from "node:crypto";\n');

  // match single-line with nested parens
  content = content.replace(/createHash\([\"'](?:sha256|sha1)[\"']\)\s*\.update\((.*?)\)\s*\.digest\([\"']hex[\"']\)/g, 'hashString($1)');

  // match multi-line
  content = content.replace(/createHash\([\"'](?:sha256|sha1)[\"']\)\s*\n\s*\.update\((.*?)\)\s*\n\s*\.digest\([\"']hex[\"']\)/g, 'hashString($1)');
