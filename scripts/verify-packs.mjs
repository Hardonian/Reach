...
async function lintPack(packPath) {
  ...
  try {
    ...
    if (typeof pack.deterministic !== 'boolean') errors.push('Missing or invalid deterministic flag');
    ...
    return { passed: false, errors: [`Failed to parse pack.json: ${err.message}`] };
  }
}
...
async function testPack(packPath) {
  ...
  try {
    ...
    return { passed: true, warning: 'No tests directory (optional but recommended)' };
  }
}
