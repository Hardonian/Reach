...
async function checkRouteBloat() {
  ...
  if (illegalRoutes.length > 0) {
    console.error(
      `❌ ENTROPY DETECTED: Illegal top-level routes found: ${illegalRoutes.join(", ")}`,
    );
    console.error(
      `Every new primary route requires Founder approval and entry in the "Allowed Routes" ledger.`,
    );
    return false;
  }
  ...
  if (routes.length > CONFIG.maxRoutes) {
    console.error(
      `❌ ENTROPY DETECTED: Route limit exceeded (${routes.length}/${CONFIG.maxRoutes}).`,
    );
    return false;
  }
