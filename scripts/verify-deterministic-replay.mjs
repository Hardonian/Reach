...
const TEST_FIXTURES = {
  ...
  constrainedDecision: {
    spec: {
      ...
      constraints: [{ type: "threshold", target: "risk", max: 0.5 }],
...
function simulateDeterministicExecution(fixture) {
  ...
  if (spec.objectives[0]?.target === "cost" || spec.objectives[0]?.target === "minimize") {
    ...
    selectedAction = spec.actions.reduce(
      (min, a) => (a.cost < min.cost ? a : min),
      spec.actions[0],
    );
    ...
    const validActions =
      spec.constraints.length > 0
        ...
        : spec.actions;
    selectedAction = validActions.reduce(
      (max, a) => ((a.value || 0) > (max.value || 0) ? a : max),
      validActions[0],
    );
...
async function testCanonicalJsonInvariants() {
  ...
  const testCases = [
    ...
    {
      name: "Mixed types",
      input: { str: "test", num: 42, bool: true, nil: null },
      expected: '{"bool":true,"nil":null,"num":42,"str":"test"}',
    },
  ];
...
async function main() {
  ...
  const allPassed =
    jsonResults.passed === jsonResults.total && hashResults.stable && results.summary.fail === 0;
