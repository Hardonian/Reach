...
for (let file of files) {
  ...
  content = content.replace(
    /process\.env\.ZEO_LOG_REDACT(?:\s+as\s+RedactMode)?(?:\s*\|\|\s*\"safe\")?/g,
    "loadConfig().ZEO_LOG_REDACT",
  );
  content = content.replace(
    /process\.env\.ZEO_LOG_FORMAT\s*===\s*\"json\"/g,
    'loadConfig().ZEO_LOG_FORMAT === "json"',
  );
  content = content.replace(/process\.env\.CI\s*===\s*\"true\"/g, "loadConfig().CI");

  content = content.replace(
    /process\.env\.ZEO_FIXED_TIME(!?)(?:\s*\|\|\s*new Date\(\)\.toISOString\(\))?/g,
    "loadConfig().ZEO_FIXED_TIME$1",
  );

  content = content.replace(/process\.env\.NODE_ENV/g, "loadConfig().NODE_ENV");
  content = content.replace(
    /process\.env\.(?:ZE0|ZEO)_STRICT(?:\s*\|\|\s*\"[0]?[false]?\")?/g,
    "loadConfig().ZEO_STRICT",
  );

  content = content.replace(
    /\{\s*\.\.\.process\.env(?:,\s*PORT\s*:\s*port)?\s*\}/g,
    "{ ...process.env, PORT: loadConfig().PORT || port }",
  );

  content = content.replace(
    /process\.env\.ZEO_WORKSPACE_ID\?\.trim\(\)(?:\s*\|\|\s*"default")?/g,
    "loadConfig().ZEO_WORKSPACE_ID",
  );

  content = content.replace(
    /process\.env\.ZEO_SIGNING_HMAC_KEY/g,
    "loadConfig().ZEO_SIGNING_HMAC_KEY",
  );
  ...
  content = content.replace(
    ...
    "loadConfig().SUPABASE_SERVICE_KEY",
  );
  ...
  content = content.replace(
    /process\.env\.ZEO_MODEL(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().ZEO_MODEL",
  );
  content = content.replace(
    /process\.env\.ZEO_PROVIDER(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().ZEO_PROVIDER",
  );
  content = content.replace(
    /process\.env\.KEYS_MODEL(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().KEYS_MODEL",
  );
  content = content.replace(
    /process\.env\.KEYS_PROVIDER(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().KEYS_PROVIDER",
  );
  content = content.replace(
    /process\.env\.READYLAYER_MODEL(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().READYLAYER_MODEL",
  );
  content = content.replace(
    /process\.env\.READYLAYER_PROVIDER(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().READYLAYER_PROVIDER",
  );
  content = content.replace(
    /process\.env\.SETTLER_MODEL(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().SETTLER_MODEL",
  );
  content = content.replace(
    /process\.env\.SETTLER_PROVIDER(?:\s*\|\|\s*"[^"]+")?/g,
    "loadConfig().SETTLER_PROVIDER",
  );
