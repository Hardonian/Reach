...
replaceFile('src/cli/workflow-cli.ts', content => {
  ...
  c = c.replace(/import \{ hashString \} from "\.\.\/determinism\/index\.js";/, 'import { hashString, hashBuffer } from "../determinism/index.js";');

  // ZEO_FIXED_TIME missing fallback
  c = c.replace(/return loadConfig\(\)\.ZEO_FIXED_TIME;/g, 'return loadConfig().ZEO_FIXED_TIME || new Date().toISOString();');
  c = c.replace(/Date\.parse\(loadConfig\(\)\.ZEO_FIXED_TIME\)/g, 'Date.parse(loadConfig().ZEO_FIXED_TIME!)');
...
replaceFile('src/cli/studio-cli.ts', content => {
  return content.replace(/PORT:\s*loadConfig\(\)\.PORT\s*\|\|\s*port/g, 'PORT: String(loadConfig().PORT || port)');
});
