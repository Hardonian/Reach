    ...
    <div className="p-6 lg:px-10 lg:py-8 max-w-[1600px] mx-auto w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 dark:text-yellow-400 relative overflow-hidden group">
        ...
        <div className="flex items-start gap-3 relative z-10">
          <span className="material-symbols-outlined text-yellow-600 dark:text-yellow-500 mt-0.5">
            warning
          </span>
          <div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto mt-2 sm:mt-0 relative z-10">
          <button
            type="button"
            className="px-4 py-1.5 rounded-md bg-yellow-500/10 hover:bg-yellow-500/20 text-yellow-700 dark:text-yellow-300 text-sm font-semibold transition-colors w-full sm:w-auto border border-yellow-500/20"
          >
            Investigate
          </button>
          <button
            type="button"
            className="p-1.5 rounded-md hover:bg-yellow-500/10 text-yellow-700 dark:text-yellow-400 transition-colors"
          >
        ...
        <div className="p-5 rounded-lg bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-[#2e3545] shadow-xs">
          ...
          <div className="mt-3 h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${health.failureRate}` }}
            ></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-[#2e3545] rounded-lg shadow-xs overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-[#2e3545]">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              Active Runs
              <span className="px-2 py-0.5 rounded-full bg-[#135bec]/10 text-[#135bec] text-xs font-bold">
                Live
              </span>
            </h3>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-[#2e3545] text-slate-600 dark:text-slate-300">
                {agents.activeRuns.map((run) => (
                  ...
                  >
                    <td className="px-6 py-3.5 font-mono text-[#135bec] group-hover:underline cursor-pointer">
                      {run.traceId}
                    </td>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="p-5 bg-white dark:bg-[#1a1f2e] border border-slate-200 dark:border-[#2e3545] rounded-lg shadow-xs">
            ...
            <div
              aria-label="Sparkline showing drift over time"
              className="h-24 w-full flex items-end gap-1 overflow-hidden"
            >
                ...
                >
                  <div className="flex items-center gap-3">
                    {repo.animate ? (
                      ...
                      <div
                        className={`size-2 rounded-full bg-${repo.color}-500 shadow-[0_0_8px_rgba(var(--${repo.color}-500-rgb),0.4)]`}
                      ></div>
                    )}
                  ...
                  <span
                    className={`text-xs font-medium text-${repo.color}-600 dark:text-${repo.color}-400`}
                  >
                    {repo.status}
                  </span>
                </div>
