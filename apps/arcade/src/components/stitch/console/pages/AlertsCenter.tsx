...
const alerts = [
  {
    id: "ALT-001",
    severity: "Critical",
    title: "Runner memory threshold exceeded",
    source: "runner-prod-3",
    time: "2m ago",
    status: "Firing",
  },
  {
    id: "ALT-002",
    severity: "High",
    title: "Evaluation accuracy dropped below 85%",
    source: "eval-engine",
    time: "14m ago",
    status: "Firing",
  },
  {
    id: "ALT-003",
    severity: "Medium",
    title: "Agent response latency elevated (p95 > 4s)",
  ...
  },
  {
    id: "ALT-004",
    severity: "Low",
    title: "Unused dataset detected — 30-day retention warning",
  ...
  },
  {
    id: "ALT-005",
    severity: "Critical",
    title: "Governance policy conflict on merge",
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-[#282e39] bg-[#111318] px-10 py-5">
        <div className="flex flex-col">
          ...
          <p className="text-[#9da6b9] text-[10px] font-bold uppercase tracking-wide">
            System health events & threshold violations
          </p>
        </div>
          ...
          <div className="flex flex-col gap-3">
            {alerts.map((alert) => {
              ...
              return (
                ...
                >
                  <div
                    className={`mt-0.5 w-2.5 h-2.5 rounded-full bg-${sColor}-500 shrink-0 ${alert.status === "Firing" ? "animate-pulse" : ""}`}
                  />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4 mb-1.5">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span
                          className={`px-2 py-0.5 rounded-full font-black uppercase tracking-widest text-[9px] bg-${sColor}-500/10 text-${sColor}-400 border border-${sColor}-500/20`}
                        >
                          {alert.severity}
                        </span>
                        <span
                          className={`px-2 py-0.5 rounded-full font-black uppercase tracking-widest text-[9px] bg-${stColor}-500/10 text-${stColor}-400 border border-${stColor}-500/20`}
                        >
                          {alert.status}
                        </span>
                      </div>
