    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-[#111418] font-sans scrollbar-hide">
      <div className="h-16 border-b border-[#3b4754] bg-[#111418]/95 backdrop-blur flex items-center justify-between px-8 sticky top-0 z-20">
        <div className="flex items-center gap-2">
          <span className="text-[#9dabb9] text-sm uppercase font-bold tracking-widest">
            Console
          </span>
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center gap-2 bg-[#1a212a] rounded-lg p-1 border border-[#3b4754]">
            <button
              type="button"
              className="px-4 py-1.5 rounded text-[10px] font-black uppercase tracking-widest bg-[#3b4754] text-white"
            >
              This Month
            </button>
            <button
              type="button"
              className="px-4 py-1.5 rounded text-[10px] font-black uppercase tracking-widest text-[#9dabb9] hover:text-white transition-all"
            >
              Last Month
            </button>
          </div>
          ...
          <div className="lg:col-span-2 bg-[#1a212a] border border-[#3b4754] rounded-2xl p-8 flex flex-col gap-6">
            ...
            <div className="h-72 w-full bg-[#111418] rounded-xl border border-[#3b4754]/50 relative flex items-end justify-between px-8 pb-4">
              ...
              {Array.from({ length: 8 }).map((_, i) => (
                <div
                  key={i}
                  className="w-[10%] h-full flex items-end justify-center relative group"
                >
                  <div className="w-full bg-slate-700 rounded-t h-[60%] relative">
                    <div
                      className="absolute bottom-0 w-full bg-[#137fec] rounded-t hover:bg-blue-400 transition-all cursor-help"
                      style={{ height: `${Math.random() * 40 + 80}%` }}
                    ></div>
                  </div>
                </div>
              ))}
              ...
              <tbody className="divide-y divide-[#3b4754] text-xs">
                {[
                  ...
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-8 py-6 text-right font-black text-white font-mono text-sm">
                      {row.total}
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="h-1 w-40 bg-slate-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full bg-${row.clr}-500 transition-all`}
                            style={{ width: `${row.pct}%` }}
                          ></div>
                        </div>
                      </div>
