    ...
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-[#101622] font-sans scrollbar-hide">
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-[#282e39] bg-[#111318] px-10 py-5">
        <div className="flex flex-col">
          <h2 className="text-white text-lg font-black uppercase tracking-widest text-[#135bec]">
            Ecosystem Coordination
          </h2>
          <p className="text-[#9da6b9] text-[10px] font-bold uppercase tracking-wide">
            Multi-repo health & global synchronization
          </p>
        </div>
      ...
      <main className="flex-1 p-8 md:p-12 max-w-[1600px] mx-auto w-full flex flex-col gap-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div
              key={node.name}
              className={`flex flex-col p-6 bg-[#1e293b] rounded-2xl border border-slate-700/50 shadow-xl relative overflow-hidden group ${node.name === "AIAS Network" ? "border-l-4 border-l-amber-500" : ""}`}
            >
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-2">
                  <div
                    className={`size-2 rounded-full bg-${node.color}-500 ${node.sync ? "animate-pulse" : ""} shadow-[0_0_8px_rgba(59,130,246,0.6)]`}
                  ></div>
                  <span className="text-xs font-black text-white uppercase tracking-widest">
                    {node.name}
                  </span>
                </div>
                <span className="text-[10px] font-mono text-slate-500 bg-[#111318] px-2 py-0.5 rounded border border-white/5">
                  {node.ver}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="bg-[#1e293b] rounded-3xl border border-slate-700/50 shadow-2xl flex flex-col">
            <div className="p-6 border-b border-slate-700/50 bg-[#111318]/30">
              ...
              <p className="text-[#9da6b9] text-[10px] uppercase font-bold tracking-wide mt-1">
                checking agents.md & parity
              </p>
            </div>
            ...
            <div className="p-6 bg-[#111318]/30 border-t border-slate-700/50">
              <button
                type="button"
                className="w-full py-3 bg-[#1c2333] border border-slate-700 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-800 transition-all"
              >
                View Full Parity Report
              </button>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-800 text-xs">
                {[
                  ...
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-8 py-5 text-[#9da6b9] font-medium uppercase tracking-widest text-[10px]">
                      {log.t}
                    </td>
                    <td className="px-8 py-5">
                      <span
                        className={`px-2 py-0.5 rounded-full font-black uppercase tracking-widest text-[9px] bg-${log.c}-500/10 text-${log.c}-400 border border-${log.c}-500/20`}
                      >
                        {log.s}
                      </span>
                    </td>
