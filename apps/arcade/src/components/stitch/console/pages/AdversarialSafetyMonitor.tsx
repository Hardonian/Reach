      ...
      <div className="p-8 flex flex-col gap-8 max-w-[1600px] mx-auto w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            ...
            <div
              key={kpi.label}
              className="bg-[#1c2632] border border-[#283442] rounded-xl p-6 relative overflow-hidden group hover:border-[#137fec]/30 transition-all"
            >
              ...
              <div
                className={`absolute bottom-0 left-0 h-1 bg-${kpi.color}-500 w-full opacity-50`}
              ></div>
            </div>
          ))}
        </div>
          ...
          <div className="lg:col-span-2 bg-[#1c2632] border border-[#283442] rounded-xl p-8 flex flex-col gap-6">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-lg font-black text-white uppercase tracking-widest">
                  PII/PHI Leakage Monitor
                </h3>
                <p className="text-sm text-[#9dabb9]">
                  Real-time scan of sensitive data traces across all agents
                </p>
              </div>
            ...
            <div className="h-64 bg-[#111821] rounded-lg border border-[#283442] relative overflow-hidden flex items-end gap-1 px-4">
              ...
              {Array.from({ length: 24 }).map((_, i) => (
                <div
                  key={i}
                  className={`flex-1 transition-all rounded-t ${i === 12 || i === 18 ? "bg-red-500/80 hover:bg-red-500" : "bg-[#137fec]/20 hover:bg-[#137fec]/40"}`}
          ...
          <div className="lg:col-span-1 bg-[#1c2632] border border-[#283442] rounded-xl p-8 flex flex-col gap-6">
            ...
            <div className="flex-1 bg-[#111821] rounded-xl border border-[#283442] relative overflow-hidden flex items-center justify-center p-4">
              ...
              <div className="absolute top-[30%] left-[50%] flex flex-col items-center">
                ...
                <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded text-[8px] font-black uppercase mt-1">
                  EU-West-1 Locked
                </span>
              </div>
            </div>
          ...
          <div className="lg:col-span-1 bg-[#1c2632] border border-[#283442] rounded-xl p-8 flex flex-col gap-6">
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={agent.name}
                  className="flex items-center gap-4 p-4 rounded-xl border border-[#283442] bg-[#111821] hover:border-[#137fec]/50 transition-all cursor-pointer"
                >
                  <div
                    className={`w-10 h-10 rounded-full border-2 border-${agent.color}-500/30 flex items-center justify-center font-black text-${agent.color}-400`}
                  >
                    {agent.grade}
                  </div>
            ...
            <button
              type="button"
              className="mt-2 w-full py-2.5 text-[10px] font-black uppercase tracking-widest text-[#9dabb9] hover:text-white border border-[#283442] rounded-xl hover:bg-[#283442] transition-colors"
            >
              Run Resilience Audit
            </button>
          </div>
        </div>
      </div>
