    ...
    <div className="relative pl-4">
      ...
      {steps.map((step, index) => (
        <React.Fragment key={step.id}>
          <div
            className={`relative flex gap-4 pb-8 group ${step.type === "subtask" ? "ml-12" : ""}`}
          >
            ...
            <div className="relative z-10 flex-none pt-2">
              <div
                className={`h-10 w-10 rounded-full bg-white dark:bg-background-dark border-2 flex items-center justify-center shadow-sm transition-all ${
                  step.status === "Running"
                    ? "border-[#135bec] shadow-[0_0_0_4px_rgba(19,91,236,0.15)]"
                    : step.status === "Success"
                      ? "border-green-500"
                      : step.status === "Warning"
                        ? "border-orange-500"
            ...
            <div
              ...
              className={`flex-1 bg-white dark:bg-[#181b21] border rounded-xl p-4 shadow-sm transition-all cursor-pointer relative overflow-hidden ${
                step.highlighted
                  ? "border-[#135bec] ring-1 ring-[#135bec]/20 shadow-lg shadow-blue-900/5"
                  : "border-slate-200 dark:border-slate-800 hover:border-[#135bec]/50 hover:shadow-md"
              }`}
            >
              {step.highlighted && (
                <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-[#135bec]"></div>
              )}
