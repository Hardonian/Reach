      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-800">
          <thead className="bg-slate-900">
            <tr>
              <th
                className="px-6 py-3 text-left text-xs font-semibold text-slate-400 border-b border-slate-800 uppercase tracking-wider cursor-pointer hover:text-slate-200"
                scope="col"
              >
              ...
              <th
                className="px-6 py-3 text-left text-xs font-semibold text-slate-400 border-b border-slate-800 uppercase tracking-wider"
                scope="col"
              >
                Repo & Version
              </th>
              ...
              <th
                className="px-6 py-3 text-center text-xs font-semibold text-slate-400 border-b border-slate-800 uppercase tracking-wider"
                scope="col"
              >
                State
              </th>
          ...
          <tbody className="divide-y divide-slate-800">
            {agents.map((agent) => (
              <tr
                key={agent.id}
                className={`hover:bg-slate-800/50 transition-colors group ${!agent.isEnabled ? "opacity-70" : ""}`}
              >
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex flex-wrap gap-1 max-w-[200px]">
                    {agent.capabilities.map((cap) => (
                      <span
                        key={cap}
                        className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-slate-800 border border-slate-700 text-slate-400"
                      >
                        {cap}
                      </span>
                    ))}
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`text-sm font-mono ${
                        parseFloat(agent.errorRate) > 10
                          ? "text-rose-400"
                          : parseFloat(agent.errorRate) > 1
                            ? "text-amber-400"
                            : "text-slate-300"
                      }`}
                    >
