...
const STATUS_CONFIG = {
  pass: { label: 'Pass', color: 'text-emerald-400', bg: 'bg-emerald-950/40 border-emerald-500/30', icon: '✅' },
  needs_attention: { label: 'Needs Attention', color: 'text-yellow-400', bg: 'bg-yellow-950/40 border-yellow-500/30', icon: '⚠️' },
  fail: { label: 'Fail', color: 'text-red-400', bg: 'bg-red-950/40 border-red-500/30', icon: '❌' },
};
...
export function HomepageClient({ variant }: { variant: Variant }) {
  ...
  async function runDemo() {
    setState('loading');
    await track('first_success_demo_run_started', { source: 'homepage', variant_id: variant });
      ...
      {state === 'done' && result && cfg && (
        <div className={`card border animate-slide-up ${cfg.bg} p-6`}>
          ...
          <div className="mb-6 p-3 rounded-lg bg-black/20 border border-white/5">
            <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Recommended fix</div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              ...
              onClick={() => track('cta_clicked', { source: 'homepage_demo_result', cta: 'open_playground' })}
            >
              Run on your agent →
            </Link>
            <Link
              ...
              onClick={() => track('signup_started', { source: 'homepage_demo_result', method: 'demo_save' })}
            >
              Save this run (free)
            </Link>
          </div>
