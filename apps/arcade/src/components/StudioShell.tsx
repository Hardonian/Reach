...
type StudioState = {
  ...
  runHistory: Array<{ command: string; timestamp: string; runId?: string; ok: boolean }>;
};
...
const federationNodes: FederationNode[] = [
  { nodeId: 'local-alpha', trustScore: 94, p50Ms: 14, p95Ms: 26, successCount: 500, failureCount: 2, quarantined: false },
  { nodeId: 'mesh-beta', trustScore: 58, p50Ms: 39, p95Ms: 90, successCount: 234, failureCount: 31, quarantined: true },
];
...
export function StudioShell() {
  ...
  const [runHistory, setRunHistory] = useState<Array<{ command: string; timestamp: string; runId?: string; ok: boolean }>>([]);
  const [commandOutput, setCommandOutput] = useState('Ready. Studio runs local-first commands via reachctl.');
  ...
  const arenaScore = useMemo(() => deterministicArenaScore(`${manifest.name}:baseline`, [0.3, 0.2, 0.25, 0.25]), [manifest.name]);
  ...
  async function persistState(nextManifest: PackManifest, nextHistory: StudioState['runHistory']) {
    await fetch('/api/studio/state', {
      ...
      body: JSON.stringify({ state: { packDraft: nextManifest, runHistory: nextHistory } }),
    }).catch(() => null);
  }
  ...
  async function runCommand(command: string) {
    ...
    const nextHistory = [{ command: data.command, timestamp, runId: data.runId, ok: data.ok }, ...runHistory].slice(0, 20);
  ...
  return (
    <main style={{ maxWidth: 1100, margin: '0 auto', padding: 16, display: 'grid', gridTemplateColumns: '220px 1fr', gap: 16 }}>
      ...
      <section aria-live="polite" style={{ border: '1px solid #2d2d2d', borderRadius: 12, padding: 16, background: '#0f0f0f' }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 12 }}>
          <label htmlFor="run-picker">Selected run</label>
          <select id="run-picker" value={selectedRun} onChange={(e) => setSelectedRun(e.target.value)}>
            <option value="">(none)</option>
            {runInventory.map((runId) => <option key={runId} value={runId}>{runId}</option>)}
          </select>
          <button disabled={busy} onClick={() => void refreshRunInventory()}>Refresh inventory</button>
        </div>
        ...
        {active === 'Packs' && (
          <>
            ...
            <div style={{ display: 'grid', gap: 8, margin: '12px 0' }}>
              <label>
                Template
                <select value={manifest.name} onChange={(e) => updateManifest((m) => ({ ...m, name: e.target.value }))} style={{ marginLeft: 8 }}>
              ...
              <label>
                specVersion
                <input value={manifest.specVersion} onChange={(e) => updateManifest((m) => ({ ...m, specVersion: e.target.value }))} style={{ marginLeft: 8 }} />
              </label>
              <label>
                Policy contract
                <input value={manifest.policyContract || ''} onChange={(e) => updateManifest((m) => ({ ...m, policyContract: e.target.value }))} style={{ marginLeft: 8, minWidth: 320 }} />
              </label>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button disabled={busy} onClick={() => runCommand('packs.init')}>init</button>
              <button disabled={busy} onClick={() => runCommand('packs.validate')}>validate</button>
              <button disabled={busy} onClick={() => runCommand('packs.sign')}>sign</button>
        ...
        {active === 'Graph' && (
          <>
            ...
            <button disabled={busy || !selectedRun} onClick={() => runCommand('graph.export')}>Export graph (json)</button>
          </>
        )}
        ...
        {active === 'Federation' && (
          <>
            <h2>Federation</h2>
            <p>Total nodes: {trust.total} • quarantined: {trust.quarantined} • avg trust: {trust.avgTrust}</p>
            <svg width="320" height="140" role="img" aria-label="Federation topology" style={{ border: '1px solid #333' }}>
              <circle cx="70" cy="70" r="24" fill="#1e7" />
              <text x="38" y="110" fill="#ddd" fontSize="12">local-alpha</text>
              <circle cx="230" cy="70" r="24" fill="#d75" />
              <text x="200" y="110" fill="#ddd" fontSize="12">mesh-beta</text>
              <line x1="94" y1="70" x2="206" y2="70" stroke="#8aa" strokeWidth="2" />
            </svg>
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button disabled={busy} onClick={() => runCommand('federation.status')}>Refresh status</button>
              <button disabled={busy} onClick={() => runCommand('federation.map')}>Export topology</button>
            </div>
          </>
        )}
        ...
        {active === 'Playground' && (
          <>
            ...
            <button disabled={busy} onClick={() => runCommand('playground.export')}>Export demo</button>
          </>
        )}
        ...
        {active === 'Support' && (
          <>
            <h2>Support</h2>
            <p>Use local KB-backed support for safe troubleshooting. Optional hosted LLM remains opt-in via env key.</p>
            <div style={{ display: 'flex', gap: 8 }}>
        ...
        <pre style={{ whiteSpace: 'pre-wrap', background: '#090909', padding: 12, borderRadius: 8 }}>
          {runHistory.length === 0 ? 'No snapshots yet.' : JSON.stringify(runHistory, null, 2)}
        </pre>

        <h3>Command output</h3>
        <pre style={{ whiteSpace: 'pre-wrap', background: '#090909', padding: 12, borderRadius: 8 }}>{busy ? 'Running…' : commandOutput}</pre>
      </section>
    </main>
  );
}
