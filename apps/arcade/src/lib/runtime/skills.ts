...
export const BUILTIN_SKILLS: SkillManifest[] = [
  {
    ...
    icon: '▶',
    inputs: [
      { name: 'agent_trace', type: 'json', required: true, description: 'Agent execution trace to evaluate' },
      { name: 'threshold', type: 'number', required: false, description: 'Minimum passing score (0-100)', default: 80 },
    ],
    tools: ['trace-parser', 'policy-engine', 'schema-validator'],
    modelHints: [
      { provider: 'anthropic', model: 'claude-sonnet-4-20250514', reason: 'Best balance of speed and accuracy for evals' },
    ],
  ...
  },
  {
    ...
    name: 'Policy Gate',
    version: '1.0.0',
    description: 'Validate tool calls against defined rules. Catches privilege escalation and banned patterns.',
    icon: '🛡',
    inputs: [
      { name: 'tool_calls', type: 'json', required: true, description: 'Array of tool call records' },
      { name: 'policy_config', type: 'json', required: false, description: 'Custom policy rules' },
    ],
  ...
  },
  {
    ...
    icon: '⟳',
    inputs: [
      { name: 'current_run', type: 'json', required: true, description: 'Current run data' },
      { name: 'baseline_run', type: 'json', required: false, description: 'Baseline run for comparison' },
    ],
  ...
  },
  {
    ...
    icon: '🔍',
    inputs: [
      { name: 'agent_id', type: 'string', required: true, description: 'Agent identifier to trace' },
    ],
  ...
  },
  {
    ...
    icon: '🚀',
    inputs: [
      { name: 'agent_trace', type: 'json', required: true, description: 'Agent execution trace' },
      { name: 'min_score', type: 'number', required: false, description: 'Minimum score to pass', default: 80 },
      { name: 'block_high_severity', type: 'boolean', required: false, description: 'Block on any high-severity finding', default: true },
    ],
  ...
  },
  {
    ...
    icon: '🔌',
    inputs: [
      { name: 'server_url', type: 'string', required: true, description: 'MCP server endpoint URL' },
      { name: 'auth_token', type: 'string', required: false, description: 'Bearer token for server auth' },
    ],
