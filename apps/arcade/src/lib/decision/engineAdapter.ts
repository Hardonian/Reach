...
export const DecisionResultSchema = z.object({
  ...
  // Governance badges
  governanceBadges: z
    .array(z.enum(["policy_conflict", "low_trust_risk", "drift_high_impact"]))
    .optional(),
...
class TSReferenceEngine implements DecisionEngine {
  ...
  private computeRiskScore(input: DecisionInput, actionId: string): number {
    ...
    if (input.policyViolations) {
      const avgViolationSeverity =
        input.policyViolations.reduce((sum, v) => sum + v.severity, 0) /
  ...
  private generateTrace(
    ...
    if (bestAction) {
      trace.push(
        `Selected action: ${bestAction.name} (score: ${bestAction.compositeScore.toFixed(2)})`,
      );
    }
