...
export function createSession(userId: string, tenantId?: string): WebSession {
  ...
  db.prepare(
    `INSERT INTO web_sessions (id, user_id, tenant_id, expires_at, created_at) VALUES (?,?,?,?,?)`,
...
export function getSession(id: string): WebSession | undefined {
  const db = getDB();
  return db
    .prepare("SELECT * FROM web_sessions WHERE id=? AND expires_at > ?")
    .get(id, new Date().toISOString()) as WebSession | undefined;
}
...
export function lookupApiKey(rawKey: string): { key: ApiKey; tenant: Tenant } | undefined {
  ...
  const row = db
    .prepare(
      `SELECT k.*, t.name as tenant_name, t.slug as tenant_slug, t.plan as tenant_plan
    FROM api_keys k JOIN tenants t ON t.id = k.tenant_id
    WHERE k.key_hash=? AND k.revoked_at IS NULL AND t.deleted_at IS NULL`,
    )
...
export function revokeApiKey(id: string, tenantId: string): boolean {
  const db = getDB();
  const res = db
    .prepare("UPDATE api_keys SET revoked_at=? WHERE id=? AND tenant_id=?")
