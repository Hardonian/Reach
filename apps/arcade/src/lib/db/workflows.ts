...
export function listProjects(tenantId: string): Project[] {
  ...
  return db
    .prepare(
      "SELECT * FROM projects WHERE tenant_id=? AND deleted_at IS NULL ORDER BY created_at DESC",
    )
...
export function listWorkflows(tenantId: string, projectId?: string): Workflow[] {
  ...
  if (projectId) {
    return db
      .prepare(
        "SELECT * FROM workflows WHERE tenant_id=? AND project_id=? AND deleted_at IS NULL ORDER BY updated_at DESC",
      )
      .all(tenantId, projectId) as Workflow[];
  }
  return db
    .prepare(
      "SELECT * FROM workflows WHERE tenant_id=? AND deleted_at IS NULL ORDER BY updated_at DESC",
    )
    .all(tenantId) as Workflow[];
}
...
export function createWorkflowRun(
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO workflow_runs (id, tenant_id, workflow_id, status, inputs_json, outputs_json, metrics_json, created_at)
    VALUES (?,?,?,'queued',?,'{}','{}',?)`,
...
export function updateWorkflowRun(
  ...
  db.prepare(
    ...
    patch.status ?? null,
    patch.outputs_json ?? null,
    patch.metrics_json ?? null,
    patch.error ?? null,
...
export function listWorkflowRuns(tenantId: string, workflowId?: string, limit = 50): WorkflowRun[] {
  ...
  if (workflowId) {
    return db
      .prepare(
        "SELECT * FROM workflow_runs WHERE tenant_id=? AND workflow_id=? ORDER BY created_at DESC LIMIT ?",
      )
