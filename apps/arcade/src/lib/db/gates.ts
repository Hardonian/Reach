...
import { type Gate, type GateRun, type GithubInstallation, type CiIngestRun, type GateReport } from './types';
...
export function createGate(tenantId: string, input: Partial<Gate>): Gate {
  ...
  db.prepare(`INSERT INTO gates (id, tenant_id, name, repo_owner, repo_name, default_branch, trigger_types, required_checks, thresholds, status, created_at, updated_at)
    ...
    .run(id, tenantId, input.name, input.repo_owner, input.repo_name,
      input.default_branch ?? 'main', JSON.stringify(input.trigger_types ?? ['pr', 'push']),
      JSON.stringify(input.required_checks ?? []), JSON.stringify(input.thresholds ?? { pass_rate: 1.0, max_violations: 0 }), now, now);
...
export function updateGate(id: string, tenantId: string, patch: Partial<Gate>): boolean {
  ...
  db.prepare(`UPDATE gates SET name=?,default_branch=?,trigger_types=?,required_checks=?,thresholds=?,status=?,updated_at=? WHERE id=? AND tenant_id=?`)
    .run(patch.name ?? existing.name, patch.default_branch ?? existing.default_branch,
      ...
      patch.status ?? existing.status, now, id, tenantId);
  return true;
}
