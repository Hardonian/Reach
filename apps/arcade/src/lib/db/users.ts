...
export function getUserById(id: string): User | undefined {
  ...
  return db
    .prepare(
      "SELECT id, email, display_name, created_at, deleted_at FROM users WHERE id=? AND deleted_at IS NULL",
    )
...
export function addMember(tenantId: string, userId: string, role: Role = "member"): void {
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT OR REPLACE INTO memberships (id, tenant_id, user_id, role, created_at) VALUES (?,?,?,?,?)`,
...
export function getMembership(tenantId: string, userId: string): { role: Role } | undefined {
  ...
  return db
    ...
    .get(tenantId, userId) as { role: Role } | undefined;
}
