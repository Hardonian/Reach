...
export function createPack(
  ...
  db.prepare(
    `INSERT INTO packs (id, tenant_id, name, slug, description, short_description, category,
...
export function publishPackVersion(
  ...
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO pack_versions (id, pack_id, version, manifest_json, readme, changelog, published_at)
    VALUES (?,?,?,?,?,?,?)`,
  ).run(id, packId, version, manifestJson, readme, changelog, now);
  db.prepare("UPDATE packs SET latest_version=?, updated_at=? WHERE id=?").run(
    version,
    now,
    packId,
  );
...
export function getPackVersion(packId: string, version: string): PackVersion | undefined {
  ...
  return db
    ...
    .get(packId, version) as PackVersion | undefined;
}
