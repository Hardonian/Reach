...
export default function ErrorsPage() {
  const commonErrors = [
    { code: 'ERR_POL_001', title: 'Capability Violation', desc: 'The agent attempted to call a tool not listed in its signed pack manifest.' },
    { code: 'ERR_DET_402', title: 'Drift Detected', desc: 'The execution core detected non-deterministic behavior during a replay attempt.' },
    { code: 'ERR_NET_503', title: 'MCP Timeout', desc: 'An MCP tool server failed to respond within the policy-defined timeout boundary.' },
    { code: 'ERR_AUTH_403', title: 'Token Expired', desc: 'The session bearer token has expired or the signature is invalid.' },
  ];
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-white/5">
            {commonErrors.map((err) => (
              <tr key={err.code}>
                <td className="py-4 pr-4 font-mono text-accent text-sm whitespace-nowrap">{err.code}</td>
