    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      <header className="flex justify-between items-end border-b border-arcade-border pb-6">
        <div>
          ...
          <p className="text-arcade-text-secondary mt-1">Status: Operational · Anti-Entropy Active</p>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-lg">
            ...
            <p className="text-xs text-arcade-text-secondary mt-1">
              "Orphan Route Found: `/console/ecosystem/repo-sync`. Deprecation recommended by Friday."
            </p>
          </div>
        </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-arcade-surface-hover border-b border-arcade-border">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium text-arcade-text-secondary text-center">Score</th>
                ...
                {decisions.map((dec: any) => (
                  <tr key={dec.id} className="hover:bg-arcade-surface-hover transition-colors">
                    <td className="px-4 py-4">
                      ...
                      <div className="text-xs text-arcade-text-tertiary truncate max-w-xs">{dec.description}</div>
                    </td>
                    <td className="px-4 py-4 text-center">
                      <span className={`px-2 py-1 rounded font-mono text-xs ${dec.score_total > 25 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-arcade-accent/20 text-arcade-accent'}`}>
                    ...
                    <td className="px-4 py-4 text-xs font-bold uppercase">
                      <span className={
                        dec.status === 'go' ? 'text-emerald-400' : 
                        dec.status === 'kill' ? 'text-red-400' : 'text-arcade-text-tertiary'
                      }>
                        {dec.status}
                      </span>
                    </td>
  ...
  return (
    <div className="bg-arcade-surface border border-arcade-border p-5 rounded-xl space-y-2 group hover:border-arcade-primary/50 transition-all">
      ...
      <div className="flex justify-between items-center text-[10px] font-medium">
        ...
        <span className={status === 'good' ? 'text-emerald-400' : 'text-arcade-accent'}>{trend}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs font-medium">
        ...
        <span className={status === 'warning' ? 'text-arcade-accent' : 'text-emerald-400'}>{value} / {limit}</span>
      </div>
