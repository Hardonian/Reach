      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-slate-50 dark:bg-slate-800">
            <th className="px-3 py-2 text-left font-semibold text-slate-600 dark:text-slate-300">Path</th>
            <th className="px-3 py-2 text-left font-semibold text-red-600 dark:text-red-400">Current</th>
            <th className="px-3 py-2 text-left font-semibold text-emerald-600 dark:text-emerald-400">Incoming</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
          {diffs.map((d) => (
            <tr key={d.path} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
              ...
              <td className="px-3 py-2 font-mono text-red-600 dark:text-red-400 break-all">{JSON.stringify(d.oldValue)}</td>
              <td className="px-3 py-2 font-mono text-emerald-600 dark:text-emerald-400 break-all">{JSON.stringify(d.newValue)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function ConfigAsCodePage() {
  const [currentSnapshot] = useState<ConfigSnapshot>(() => loadStoredSnapshot() ?? createDefaultSnapshot());
  ...
  return (
    <ConsoleLayout>
      <div className="p-6 lg:p-10 max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Configuration Snapshot</h1>
        ...
        <div className="flex flex-col gap-6">
          ...
          <section className="border border-slate-200 dark:border-slate-700 rounded-xl p-6 bg-white dark:bg-slate-900">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-3">Export Current Config</h2>
            <p className="text-sm text-slate-500 mb-4">Download the current configuration as a JSON snapshot. No secrets are included.</p>
            <button
              type="button"
          ...
          {/* Import */}
          <section className="border border-slate-200 dark:border-slate-700 rounded-xl p-6 bg-white dark:bg-slate-900">
            <h2 className="text-sm font-bold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-3">Import Config</h2>
            <p className="text-sm text-slate-500 mb-4">Upload a JSON snapshot to preview changes before applying.</p>
            <input
              type="file"
              accept=".json"
            ...
            {importDiffs && (
              <div className="mt-4 flex flex-col gap-4">
                <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                  Diff Preview ({importDiffs.length} change{importDiffs.length !== 1 ? 's' : ''})
                </h3>
