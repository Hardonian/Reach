...
interface Gate {
  id: string; name: string; repo_owner: string; repo_name: string;
  default_branch: string; status: 'enabled' | 'disabled';
  trigger_types: string[]; required_checks: unknown[]; created_at: string;
}
...
function StatusBadge({ status }: { status: 'enabled' | 'disabled' }) {
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
      ...
      <span className={`w-1.5 h-1.5 rounded-full ${status === 'enabled' ? 'bg-green-400' : 'bg-gray-400'}`} />
      {status}
    </span>
  );
}
...
export default function ReleaseGatesPage() {
  ...
  const [form, setForm] = useState({ name: '', repo_owner: '', repo_name: '', default_branch: 'main' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetch('/api/v1/gates').then((r) => r.json()).then((d) => {
      setGates(d.gates ?? []);
      setLoading(false);
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      setShowCreate(false);
      setForm({ name: '', repo_owner: '', repo_name: '', default_branch: 'main' });
    ...
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Block deployments when agent readiness checks fail.</p>
        </div>
        <button
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="Production gate" required
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1">Repo owner</label>
              <input
                value={form.repo_owner} onChange={(e) => setForm((f) => ({ ...f, repo_owner: e.target.value }))}
                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm text-white"
      ...
      {/* Gate list */}
      {loading ? (
        <div className="flex items-center gap-2 text-gray-400 text-sm py-8"><span>Loading gates…</span></div>
      ) : gates.length === 0 ? (
        <div className="text-center py-16">
          <span className="material-symbols-outlined text-4xl text-gray-600 block mb-3">verified</span>
        ...
        <div className="flex flex-col gap-3">
          {gates.map((gate) => (
            <div key={gate.id} className="flex items-center justify-between p-4 rounded-xl border border-border bg-surface hover:bg-surface/80">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => runGate(gate.id)}
      ...
      {gates.length > 0 && (
        <div className="mt-6 p-4 rounded-xl border border-border bg-surface/50">
          ...
          <p className="text-xs text-gray-500 mt-2">Set this as your repo webhook URL. Use secret from GITHUB_WEBHOOK_SECRET.</p>
        </div>
      )}
    </div>
  );
