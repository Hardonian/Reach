...
export default function SystemStatusPage() {
  const systems = [
    { name: 'Runner Engine', status: 'Operational', uptime: '99.99%', region: 'Global' },
  ...
  const incidents = [
    { date: 'Feb 18, 2026', title: 'Registry Latency Spike', resolved: true, description: 'Brief latency spike in marketplace resolution due to peak routing load. Mitigated via auto-scaling.' },
    { date: 'Feb 12, 2026', title: 'Scheduled Maintenance', resolved: true, description: 'Upgraded deterministic core to v2.4.0. Completed with zero downtime.' },
  ];
      ...
      <div className="max-w-4xl mx-auto">
        <header className="mb-16 flex flex-col md:flex-row md:items-center justify-between gap-8">
          ...
          <div className="px-6 py-4 bg-green-500/10 border border-green-500/30 rounded-2xl flex items-center gap-4">
            ...
            <span className="text-green-500 font-bold uppercase tracking-widest text-sm">All Systems Operational</span>
          </div>
        </header>
              ...
              <tbody className="divide-y divide-white/5">
                {systems.map((s) => (
                  <tr key={s.name} className="group hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-8 py-6 text-sm text-gray-400">{s.uptime}</td>
                    <td className="px-8 py-6 text-xs text-gray-500 font-mono italic">{s.region}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {incidents.map((i, index) => (
              <div key={index} className="p-8 bg-white/5 border border-white/10 rounded-3xl">
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-xs text-gray-500 font-mono">{i.date}</span>
                  <span className="px-2 py-0.5 bg-green-500/10 text-green-500 text-[10px] rounded border border-green-500/20 font-bold uppercase tracking-tight">Resolved</span>
                </div>
                <h3 className="text-xl font-bold mb-3">{i.title}</h3>
                <p className="text-sm text-gray-400 leading-relaxed italic">{i.description}</p>
              </div>
            ))}
          </div>
        </section>
