...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  auditLog(ctx, 'gate_run.create', 'gate_run', gateRun.id, { gate_id: id, trigger_type: parsed.data.trigger_type }, req);
  ...
  void runGate(ctx.tenantId, gateRun.id).catch((err) => {
    console.error('Gate run failed', { gate_run_id: gateRun.id, err: String(err) });
  });
