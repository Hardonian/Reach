...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({
      ok: true,
      message: 'If an account exists (or will be created), you will receive a sign-in link shortly.',
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const {
      getUserByEmail, createUser, createTenant, addMember,
      createSession, getTenantBySlug, listTenantsForUser,
    ...
    if (user) {
      ...
      const slug = email.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '-').slice(0, 32);
