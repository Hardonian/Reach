...
async function simulateExecution(
  runId: string,
  tenantId: string,
  wf: { id: string; name: string; graph_json: string },
  inputs: unknown,
) {
  ...
  try {
    const resp = await fetch(`${runnerUrl}/v1/runs`, {
      ...
      body: JSON.stringify({
        capabilities: graph.nodes?.map((n: { type: string }) => n.type) ?? [],
        inputs,
      }),
    ...
    /* runner not available, use simulation */
  }
