...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  return NextResponse.json({ workflow: { ...wf, graph: JSON.parse(wf.graph_json) } });
}
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  auditLog(ctx, 'workflow.update', 'workflow', id, { patch: Object.keys(patch).filter((k) => patch[k as keyof typeof patch] !== undefined) }, req);
  const wf = getWorkflow(id, ctx.tenantId)!;
  return NextResponse.json({ workflow: { ...wf, graph: JSON.parse(wf.graph_json) } });
}
