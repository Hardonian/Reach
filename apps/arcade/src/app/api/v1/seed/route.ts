...
export async function POST(_req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const SAMPLE_PACKS = [
      {
        name: 'Web Research Agent', slug: 'web-research-agent',
      ...
      {
        name: 'Data Analysis Pack', slug: 'data-analysis-pack',
        ...
        category: 'data' as const, visibility: 'public' as const,
        tools: ['file.read', 'data.process', 'chart.generate'],
        tags: ['csv', 'excel', 'analytics'], permissions: ['fs:read', 'memory:high'],
        dataHandling: 'processed' as const, authorName: 'DataFlow Inc',
        version: '3.2.1', readme: '# Data Analysis Pack\n\nAnalyze your data with AI-powered insights.',
        changelog: 'Fixed Excel parsing', reputationScore: 94, downloads: 8930,
      },
      {
        name: 'Code Review Agent', slug: 'code-review-agent',
        description: 'Automated PR review with security scanning and best practice recommendations.',
        shortDescription: 'Automated PR review and security scanning',
        category: 'development' as const, visibility: 'public' as const,
        tools: ['git.read', 'code.analyze', 'security.scan'],
        tags: ['code-review', 'security', 'devops'], permissions: ['git:read', 'network:external'],
        dataHandling: 'minimal' as const, authorName: 'DevTools Co',
        version: '4.0.2', readme: '# Code Review Agent\n\nAI-powered code review for modern development teams.',
        changelog: 'Added multi-language support', reputationScore: 96, downloads: 12300,
      },
    ];
    ...
    for (const p of SAMPLE_PACKS) {
      try {
        ...
        db.prepare('UPDATE packs SET reputation_score=?, downloads=?, security_status=?, verified=1 WHERE id=?')
    ...
    return NextResponse.json({
      ...
      note: rawApiKey === 'ALREADY_SEEDED' ? 'Tenant already seeded.' : 'Dev seed created. Store your API key — shown only once.',
    });
  } catch (err) {
