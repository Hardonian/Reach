...
import { listTurbulence, type RunRecord } from '@/lib/dgl-runs-api';
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  return NextResponse.json({ ok: true, data: listTurbulence(record, { page: Number(u.searchParams.get('page') ?? 1), limit: Number(u.searchParams.get('limit') ?? 20), severity: u.searchParams.get('severity') ?? '', pathPrefix: u.searchParams.get('pathPrefix') ?? '' }) });
}
