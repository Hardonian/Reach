...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const timeline = [
      { timestamp: Date.now(), type: 'policy.gate.check', status: 'success', details: 'Allow policies: ' + (pack.policyConstraints?.join(', ') || 'none') },
      { timestamp: Date.now() + 50, type: 'execution.queued', status: 'pending', id: `run-${Math.random().toString(36).substring(7)}` },
