    ...
    <div className="section-container py-12">
      ...
      <div className="max-w-3xl mx-auto mb-10 text-center">
        ...
        <p className="text-gray-400 max-w-lg mx-auto">
          Composable units of agent behavior. Each skill declares inputs, tools, and evaluation hooks.
        </p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {skills.map((skill) => {
            ...
            return (
              <button
                key={skill.id}
                onClick={() => { setSelectedSkill(skill); setShowMCP(false); }}
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-2">
                    {skill.tags.filter((t) => TAG_COLORS[t]).map((t) => (
                      <span key={t} className={`text-xs px-2 py-0.5 rounded-full ${TAG_COLORS[t]}`}>
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500">
                  <span>{skill.inputs.length} input{skill.inputs.length !== 1 ? 's' : ''}</span>
                  <span>{tools.length} tool{tools.length !== 1 ? 's' : ''}</span>
                  <span>{skill.evaluationHooks.length} eval hook{skill.evaluationHooks.length !== 1 ? 's' : ''}</span>
                </div>
              </button>
            );
          })}
          ...
          {selectedSkill ? (
            <div className="card sticky top-24">
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedSkill.tools.map((t) => (
                    <span key={t} className="text-xs px-2 py-1 rounded bg-surface border border-border font-mono">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              {selectedSkill.modelHints.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-xs text-gray-500 uppercase tracking-wide mb-2">Model Hints</h3>
              ...
              {selectedSkill.evaluationHooks.length > 0 && (
                <div className="mb-4">
                  <h3 className="text-xs text-gray-500 uppercase tracking-wide mb-2">Evaluation Hooks</h3>
                  <div className="flex flex-wrap gap-1">
                    {selectedSkill.evaluationHooks.map((h) => (
                      <span key={h} className="text-xs px-2 py-1 rounded bg-emerald-950/30 text-emerald-400 font-mono">
                        {h}
                      </span>
                    ))}
                  </div>
