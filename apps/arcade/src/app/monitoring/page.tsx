...
interface Signal {
  ...
  status: 'enabled' | 'disabled'; created_at: string;
}
interface MonitorRun {
  id: string; signal_id: string; value: number; alert_triggered: boolean; created_at: string;
}
interface Health { total: number; alerts_today: number; latest_drift: number }

function MetricCard({ label, value, sub, alert }: { label: string; value: string | number; sub?: string; alert?: boolean }) {
  return (
    <div className={`p-4 rounded-xl border ${alert ? 'border-orange-500/30 bg-orange-500/5' : 'border-border bg-surface'}`}>
...
export default function MonitoringPage() {
  ...
  async function toggleSignal(signal: Signal) {
    ...
    if (res.ok) setSignals((prev) => prev.map((s) => s.id === signal.id ? { ...s, status: next } : s));
  }

  const driftLabel = health.latest_drift > 0.5 ? '⚠ High' : health.latest_drift > 0.2 ? 'Moderate' : 'Stable';
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            <Link href={ROUTES.SETTINGS.ADVANCED.ALERTS}
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <MetricCard label="Alerts today" value={health.alerts_today} sub="threshold breaches" alert={health.alerts_today > 0} />
          <MetricCard label="Drift score" value={typeof health.latest_drift === 'number' ? health.latest_drift.toFixed(2) : '—'} sub={driftLabel} alert={health.latest_drift > 0.5} />
        </div>
            ...
            <form onSubmit={handleCreateSignal} className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                  ...
                  placeholder="Production latency" required />
              </div>
              <div>
              ...
              <div className="col-span-3 flex gap-3">
                ...
                <button type="button" onClick={() => setShowCreate(false)} className="text-sm text-gray-400">Cancel</button>
              </div>
            </form>
          </div>
        )}
              ...
              <div key={signal.id} className="p-4 rounded-xl border border-border bg-surface">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="material-symbols-outlined text-[16px] text-accent">sensors</span>
                    <span className="text-sm font-medium text-white">{signal.name}</span>
                    <span className="px-1.5 py-0.5 rounded text-xs bg-surface border border-border text-gray-400">{signal.type}</span>
                    <span className={`px-1.5 py-0.5 rounded text-xs ${
                      signal.status === 'enabled' ? 'bg-green-500/15 text-green-400' : 'bg-gray-500/15 text-gray-400'
                    }`}>{signal.status}</span>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div className="text-xs text-gray-500">
                  Ingest via: <code className="font-mono text-gray-400">POST /api/monitor/ingest</code> with signal_id: <code className="font-mono text-gray-400">{signal.id}</code>
                </div>
              </div>
            ))}
          </div>
