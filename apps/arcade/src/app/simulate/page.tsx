...
const PRESETS: Record<
  string,
  { label: string; description: string; defaults: Partial<ScenarioVariant>[] }
> = {
  model_ab: {
    label: "Model A/B",
  ...
  async function startSimulation() {
    ...
    const createRes = await fetch("/api/v1/scenarios", {
      ...
      body: JSON.stringify({
        name,
        variants,
        compare_metrics: ["pass_rate", "latency", "cost"],
      }),
    });
    if (!createRes.ok) {
      setRunning(false);
    ...
    // Trigger run
    const runRes = await fetch(`/api/v1/scenarios/${scenario.id}/run`, {
      method: "POST",
    });
    if (!runRes.ok) {
      setRunning(false);
    ...
    const { scenario_run } = (await runRes.json()) as {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Run what-if experiments before shipping to production.
          </p>
        </div>
                    ...
                    <tbody>
                      {activeRun.results.map((r, idx) => {
                        ...
                        return (
                          <tr
                            key={r.variant_id}
                            className="border-b border-border/50 last:border-0 relative"
                          >
