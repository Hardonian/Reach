    ...
    <div className="section-container py-12">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
        ...
        <div className="w-full md:w-80 relative">
          ...
          />
          <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {activeTab === 'templates' ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTemplates.map((template) => (
              <div key={template.id} className="card hover:border-accent/50 transition-all flex flex-col group">
                <div className="flex items-start justify-between mb-4">
                  <div className="text-3xl grayscale group-hover:grayscale-0 transition-all">{template.icon}</div>
                  <div className="flex gap-2">
                    <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-bold ${DIFFICULTY_COLORS[template.difficulty]}`}>
                      {template.difficulty}
                    </span>
                  </div>
                </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAgents.map((agent) => (
              <div key={agent.id} className="card flex flex-col group hover:border-accent/50 transition-all">
                <div className="flex items-start justify-between mb-4">
                ...
                <p className="text-sm text-gray-400 mb-6 flex-1 line-clamp-2">{agent.description}</p>
                <div className="flex gap-2">
                  <Link
