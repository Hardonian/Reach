...
interface SharedReport {
  type: 'gate_run' | 'scenario_run'; id: string; status: string;
  report?: {
    verdict: string; pass_rate: number; violations: number;
    findings: Array<{ rule: string; severity: string; message: string; fix: string }>;
    summary: string;
  };
  results?: Array<{
    variant_id: string; variant_label: string; status: string;
    latency_ms: number; pass_rate: number; cost_usd: number;
  }>;
...
export default function SharedReportPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/v1/reports/share/${slug}`)
      ...
      .then((d) => { setReport(d as SharedReport); setLoading(false); })
  ...
  if (notFound || !report) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-3">
      ...
      <a href="/" className="text-sm text-accent underline">Go to ReadyLayer</a>
    </div>
  );
    ...
    <div className="min-h-screen bg-background py-10 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-8">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-widest">ReadyLayer</span>
          <span className="text-gray-600">·</span>
          <span className="text-xs text-gray-500">Shared {isGate ? 'Gate Report' : 'Simulation Report'}</span>
        </div>
          ...
          <div className="flex flex-col gap-3 mb-6">
            {report.report.findings.map((f, i) => (
              <div key={i} className={`p-4 rounded-xl border ${
                ...
                <p className="text-xs text-gray-400"><span className="text-gray-500">Fix: </span>{f.fix}</p>
              </div>
            ))}
          </div>
        ) : null}
              ...
              <thead className="border-b border-border bg-surface">
                <tr>
                  {['Variant', 'Status', 'Pass rate', 'Latency', 'Cost'].map((h) => (
                    <th key={h} className={`py-2 px-4 text-xs text-gray-400 ${h === 'Variant' ? 'text-left' : 'text-right'}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {report.results.map((r) => (
                  <tr key={r.variant_id} className="border-b border-border/50 last:border-0">
                    ...
                    <td className="py-2 px-4 text-right">
                      <span className={`text-xs ${r.status === 'passed' ? 'text-green-400' : 'text-red-400'}`}>{r.status}</span>
                    </td>
        ...
        <div className="mt-8 text-xs text-gray-600 text-center">
          ...
          {report.share_expires_at ? ` · Expires ${new Date(report.share_expires_at).toLocaleDateString()}` : ''}
          {' · '}
          <a href="https://readylayer.com" className="text-gray-500 hover:text-gray-400">ReadyLayer</a>
        </div>
      </div>
    </div>
  );
