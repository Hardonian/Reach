...
interface GateReport {
  type: 'gate_run'; id: string; status: string;
  trigger_type: string; commit_sha: string | null;
  ...
  report: {
    ...
    summary: string; report_url?: string;
  };
  created_at: string; finished_at: string | null;
}
interface SimReport {
  type: 'scenario_run'; id: string; status: string;
  results: Array<{
...
function VerdictBanner({ verdict, summary }: { verdict: 'passed' | 'failed' | string; summary: string }) {
  ...
  return (
    <div className={`flex items-center gap-4 p-5 rounded-2xl border ${
      ...
      <span className={`material-symbols-outlined text-3xl ${passed ? 'text-green-400' : 'text-orange-400'}`}>
...
export default function ReportPage() {
  ...
  async function createShare() {
    ...
    const res = await fetch(`/api/v1/reports/${id}/share`, {
      ...
      body: JSON.stringify({ resource_type: resourceType, expires_in_seconds: 604800 }), // 7 days
    });
    if (res.ok) {
  ...
  if (!report) return (
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-gray-400 text-sm mt-1">This report may have expired or been deleted.</p>
      </div>
    </div>
  );
          ...
          <div>
            ...
            {report.status === 'running' && (
              <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                <span className="animate-spin material-symbols-outlined text-[14px]">sync</span> Running…
              </p>
            )}
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3 my-4">
              <div className="p-3 rounded-lg border border-border bg-surface text-center">
                ...
                <p className="text-xl font-bold text-white">{(report.report.pass_rate * 100).toFixed(0)}%</p>
              </div>
              <div className="p-3 rounded-lg border border-border bg-surface text-center">
                <p className="text-xs text-gray-400">Violations</p>
                <p className={`text-xl font-bold ${report.report.violations > 0 ? 'text-orange-400' : 'text-white'}`}>
                ...
                <div className="flex flex-col gap-3">
                  {report.report.findings.map((f, i) => (
                    <div key={i} className={`p-4 rounded-xl border ${
                      f.severity === 'error' ? 'border-red-500/30 bg-red-500/5' : 'border-orange-500/20 bg-orange-500/5'
                    }`}>
                      <div className="flex items-start gap-2 mb-2">
                        <span className={`material-symbols-outlined text-[16px] ${f.severity === 'error' ? 'text-red-400' : 'text-orange-400'}`}>
                      ...
                      <p className="text-xs text-gray-400"><span className="text-gray-500">Suggested fix: </span>{f.fix}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {report.commit_sha && (
              <div className="mt-4 text-xs text-gray-500">
                Commit: <code className="font-mono text-gray-400">{report.commit_sha.slice(0, 8)}</code>
                {report.branch && <> · Branch: <code className="font-mono text-gray-400">{report.branch}</code></>}
                ...
                <thead className="border-b border-border">
                  <tr>
                    {['Variant', 'Status', 'Pass rate', 'Latency', 'Cost'].map((h) => (
                      <th key={h} className={`py-3 px-4 text-xs font-medium text-gray-400 ${h === 'Variant' ? 'text-left' : 'text-right'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={r.variant_id} className="border-b border-border/50 last:border-0">
                      ...
                      <td className="py-2 px-4 text-right">
                        <span className={`px-2 py-0.5 rounded text-xs ${
                          r.status === 'passed' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
                        }`}>{r.status}</span>
                      </td>
